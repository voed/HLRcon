...
package voed.voed.hlrcon.steamcondenser.steam;
...
import voed.voed.hlrcon.steamcondenser.steam.community.SteamId;
...
public class SteamPlayer {
    ...
    private boolean isRealSteam;
    private String avatarURL;
    private SteamId realsteamid;
    ...
    public void addInformation(Map<String, String> playerData)
        ...
        if(this.isBot() ||this.steamId.matches("^[A-Z]+_ID_[A-Z]+$"))
        {
            this.isRealSteam = false;
        }
        else{
            try {
                this.realsteamid = SteamId.create(SteamId.convertSteamIdToCommunityId(steamId), false);
                if(!realsteamid.checkforreal())
                {
                    return;
                }
                this.isRealSteam = true;
                realsteamid.fetchData();
                this.avatarURL = realsteamid.getAvatarFullUrl();
                this.avatar = BitmapFactory.decodeStream(new URL(avatarURL).openStream());
                this.vacBanned = realsteamid.isBanned();
                //Log.d("steamcondenser", avatarURL);
            }
            catch (Exception e)
            {
                Log.d("steamcondenser", e.toString());
            }
        }
    }
    ...
    public boolean isRealSteam()
    {
        return this.isRealSteam;
    }

    public boolean isVacBanned() { return this.vacBanned; }
    ...
    {
        return this.realsteamid.getBaseUrl();
    }
    /**
