...
import voed.voed.hlrcon.steamcondenser.steam.packets.A2S_INFO_Packet;
import voed.voed.hlrcon.steamcondenser.steam.packets.A2S_PLAYER_Packet;
import voed.voed.hlrcon.steamcondenser.steam.packets.A2S_RULES_Packet;
import voed.voed.hlrcon.steamcondenser.steam.packets.S2A_INFO_BasePacket;
import voed.voed.hlrcon.steamcondenser.steam.packets.S2A_PLAYER_Packet;
import voed.voed.hlrcon.steamcondenser.steam.packets.S2A_RULES_Packet;
import voed.voed.hlrcon.steamcondenser.steam.packets.S2C_CHALLENGE_Packet;
import voed.voed.hlrcon.steamcondenser.steam.packets.SteamPacket;
import voed.voed.hlrcon.steamcondenser.steam.sockets.QuerySocket;
...
public abstract class GameServer extends Server {
    ...
    protected void handleResponseForRequest(int requestType, boolean repeatOnFailure)
        ...
        if(!expectedResponse.isInstance(responsePacket)) {
            Logger.getLogger("voed.voed.hlrcon.steamcondenser").warning("Expected " + expectedResponse + ", got " + responsePacket.getClass() + ".");
            if(repeatOnFailure) {
