...
public class SteamId {
    ...
    public void fetchData() throws SteamCondenserException {
        try {
            //Log.d("steamcondenser", this.getBaseUrl());
    ...
    public boolean checkforreal()
    {
        try {
            ...
            if(profile.hasElement("privacyMessage")) {
                if(profile.getString("privacyMessage").contains("This user has not yet set up their Steam Community profile."))
                return false;
            }
        }
        catch (Exception e)
